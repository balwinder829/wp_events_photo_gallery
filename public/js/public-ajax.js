jQuery(document).ready(function($) {
       ...
       // Clickable Albums - Load Photos in Thumbnails
       $(document).on('click', '.album-link', function(e) {
        ...
        if (!albumId) {
            alert("The album could not be loaded. Please try again.");
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert('Could not load the photos for this album.');
                }
            },
            error: function() {
                alert('There was an error loading the album photos. Please try again.');
            }
        });
    });

    // Load More Photos
    // (Retain the rest of the original script as it was previously)
    ...
    $(document).on('click', '.epa-wishlist-load-more', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    $('.epa-wishlist-grid').append(response.data.content);
                    button.prop('disabled', false).text('Load More');
                }
                if (response.data.no_more_images == 0) {
                    button.hide();
