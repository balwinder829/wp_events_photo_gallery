...
class Ajax_Handler {
    ...
    public function load_more_from_wishlist(){
        ...
        foreach ( $photos as $photo ) : ?>
            <div class="epa-wishlist-item <?php echo $class;?>">
                ...
                <p class="epa-photo-filename"><?php echo esc_html( $file_name ); ?></p> <!-- Εμφάνιση ονόματος αρχείου -->
    ...
    public function load_album_photos() {
        if ( ! check_ajax_referer( 'epa_nonce', 'security', false ) ) {
            wp_send_json_error( 'Invalid nonce' );
            exit;
        }
        ...
        ob_start();
        echo '<div class="photo-thumbnails-grid">';
        foreach ( $photos as $photo ) {
            $thumbnail_url = wp_get_attachment_image_url( $photo->ID, 'thumbnail' );
            echo '<div class="photo-thumbnail">';
            echo '<img src="' . esc_url( $thumbnail_url ) . '" alt="" class="epa-photo-item" data-full-url="' . esc_url( wp_get_attachment_url( $photo->ID ) ) . '">';
            echo '<span class="epa-wishlist-icon" data-photo-id="' . esc_attr( $photo->ID ) . '">❤</span>';
            echo '</div>';
        }
        echo '</div>';
        $content = ob_get_clean();
